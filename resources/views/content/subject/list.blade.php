...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form class="mb-3">
            <div class="row">
                ...
                <div class="col-md-6 text-md-end mt-3 mt-md-0">
                    <a href="{{ url('term&subject/subject') }}" class="btn btn-outline-secondary">
                        <i class="bx bx-refresh"></i> Clear
                    </a>
                    <a href="{{ url('term&subject/subject/create') }}" class="btn" style="background-color:#009DE1; color: white">
        ...
        <div class="table-responsive text-nowrap" id="containlist">
            @include('content.subject.table')
        </div>
        <div class="d-flex justify-content-between align-items-center mt-3">
            <p>Total Records: {{ $subjects->total() }}</p>
            {{ $subjects->links() }}
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        $('#search').on('keyup', function() {
            ...
            $.ajax({
                type: 'get',
                url: '/term&subject/subject/search',
                data: {
                    'search': $value
                },
